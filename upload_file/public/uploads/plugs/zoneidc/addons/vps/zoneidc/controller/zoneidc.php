...
class zoneidc extends Controller{
    ...
    public function control($data=[]){
      ...
      $_URI = 'http://chshuju.comchshuju.com/api/cloudapi.asp';
      ...
      switch($data['action']){
        ...
        break;
        case 'changepw':
        $_URI = 'http://chshuju.comchshuju.com/api/cloudop.asp';
        //修改密码
        ...
        break;
        case 'manager':
          $post = '<form id="sub" action="http://chshuju.com/vpsadm/login.asp" method="post"><input type="hidden" name="vpsname" value="'.$data['data']['vpsname'].'"/><input type="hidden" name="vpspassword" value="'.$data['data']['password'].'"/></form>';
      ...
      if($data['action'] == 'activate'){
        ...
        $saveres = Db::name('gee_vps_zoneidc')->insert($saveData);
        if(!$saveres){
          $ret['status'] = 500;
          $ret['msg'] = '操作超时！';
        }
        $savevpsres = Db::name('gee_vps')->insert(['user_id'=>$data['user_id'],'plug_id'=>$data['plug_id'],'plug_type'=>'gee_vps_zoneidc','plug_name'=>'zoneidc','pro_id'=>Db::name('gee_vps_zoneidc')->where('name = "'.$res['vpsname'].'"')->find()['id'],'create_time'=>time(),'update_time'=>time()]);
      } else if($data['action'] == 'getinfo'){
        $ret['data'] = $res;
      } else if($data['action'] == 'changepw'){
        $ret['data'] = '';
        $up = Db::name('gee_vps_zoneidc')->where('name = "'.$data['data']['vpsname'].'"')->update(['password'=>$data['data']['password']]);
      } else if($data['action'] == 'renew'){
        $ret['data'] = '';
        $up = Db::name('gee_vps_zoneidc')->where('name = "'.$data['data']['vpsname'].'"')->update(['end_time'=>$res['endtime']]);
